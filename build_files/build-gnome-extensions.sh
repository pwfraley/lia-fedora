...
dnf5 -y install meson glib2-devel sassc cmake dbus-devel
...
meson setup build -Dtarget=zip && meson -C build
echo $(ls)
echo $(ls build)
cd ..
...
dnf5 -y remove meson glib2-devel sassc cmake dbus-devel
